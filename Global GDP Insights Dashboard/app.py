...
DATA_PATH = Path("data/tidy/Merged_tidy.csv")
...
if not DATA_PATH.exists():
    st.error(f"{DATA_PATH} not found. Add it to the repo and redeploy.")
    st.stop()
...
st.sidebar.markdown("---")
st.sidebar.caption("Data: World Bank WDI")

# Header
st.title("🌍 Global GDP Insights Dashboard")
st.caption(
    "Explore GDP levels and growth by country over time. "
    "[Source: World Bank Data360](https://data360.worldbank.org/en/search?search=GDP&themeAndTopics=P3)"
)
...
val_col = metric
map_df = ydf[ydf["iso3"].astype(str).str.len() == 3].copy()  # filter to real ISO-3
fig_map = px.choropleth(
    map_df, locations="iso3", color=val_col, hover_name="country",
...
if sel_countries:
    ...
    fig_ts = px.line(
        line_df, x="year", y=val_col, color="country", markers=True,
...
st.dataframe(
    rank_df[["country", val_col]].rename(columns={"country": "Country", val_col: "Value"}),
    use_container_width=True
)

st.markdown("---")
st.markdown(textwrap.dedent(f"""
**Usage**: The app reads `{DATA_PATH.as_posix()}` with columns `country, iso3, year, gdp_usd, gdp_growth(%)`.  
